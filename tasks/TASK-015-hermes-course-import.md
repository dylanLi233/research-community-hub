...
- 失败请求也记录并重放。
- 章节请求哈希包含父课程 external_id，避免跨课程错误重放。
...
- [x] 有效 `courses:write` Token 可导入课程和章节。
- [x] 缺少 Token、Scope 或 Idempotency-Key 被拒绝。
- [x] 严格 Schema 拒绝未知字段和缺失字段。
- [x] 课程 external_id 首次创建、变化更新、不变 unchanged。
- [x] 章节 external_id 首次创建、变化更新、不变 unchanged。
- [x] 章节只能导入到当前 Client 自己的课程。
- [x] 父课程不存在、归档或删除时返回明确错误。
- [x] 审核开关正确决定 pending_review 或 published。
- [x] HTML、付费墙和素材权限错误返回字段级错误。
- [x] 课程和章节 Slug 冲突返回明确错误。
- [x] 相同幂等请求重放原响应。
- [x] 幂等键冲突返回 409。
- [x] 失败请求写入导入日志且不保存正文、Token 或 Header。
- [x] external_id 查询只能读取当前 Client 自己的课程和章节。
- [x] curl 文档可直接执行。
- [x] 自动化测试覆盖 Schema、动作决策、审核状态和父子归属策略。
- [x] Next.js 同级动态路由名称冲突已清理。
- [x] Lint、Typecheck、Vitest、Migration 零漂移、两次 D1 Migration、Next.js Build 和 OpenNext Build 全部通过。

> 本任务完成课程自动导入链路。前台课程目录和章节会员阅读将在 TASK-016 接入相同数据与付费墙管道。
