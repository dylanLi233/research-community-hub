...
- [ ] courses 与 course_chapters Migration 可应用并重复执行迁移命令。
- [ ] 管理员可创建、筛选、读取和编辑课程。
- [ ] 管理员可在课程下创建、排序、读取和编辑章节。
- [ ] 非管理员、未登录和跨源写请求被拒绝。
- [ ] 课程 slug 冲突返回明确错误。
- [ ] 同课程章节 slug 冲突返回明确错误。
- [ ] 非法标签、position、预计时长和访问模式返回字段级错误。
- [ ] 课程简介和章节正文保存前完成安全清洗。
- [ ] 课程封面、简介图片与章节图片执行正确素材权限校验。
- [ ] API 不返回 R2 Object Key、导入内部字段或密码数据。
- [ ] 课程与章节 PATCH 不能直接修改状态。
- [ ] 发布与归档写入审计日志。
- [ ] 课程发布不自动发布章节。
- [ ] 章节发布要求所属课程有效。
- [ ] 内容哈希在内容相同时稳定，关键字段变化时改变。
- [ ] 自动化测试覆盖字段规范化、状态、哈希和排序。
