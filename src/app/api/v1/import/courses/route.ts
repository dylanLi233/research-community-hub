...
import { importCourseSchema } from "@/imports/course-validation";
...
const ENDPOINT = "/api/v1/import/courses";
...
export async function POST(request: NextRequest) {
  ...
  if (byteLength(rawBody) > MAX_IMPORT_BODY_BYTES) {
    const requestHash = await hashImportRequest({ endpoint: ENDPOINT, rawBody });
  ...
  const requestHash = await hashImportRequest({ endpoint: ENDPOINT, body: parsedJson });
  const replay = await replayOrConflict({
    db: authorization.db,
    apiClientId: authorization.client.id,
    idempotencyKey,
  ...
  const validated = importCourseSchema.safeParse(parsedJson);
  ...
  try {
    const result = await importCourse(authorization.db, {
      apiClientId: authorization.client.id,
      idempotencyKey,
      requestHash,
      requestId,
