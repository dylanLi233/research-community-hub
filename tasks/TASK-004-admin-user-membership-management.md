...
- 新建普通会员默认 `must_change_password=true`。
...
- [ ] 重复用户名返回 409，不泄露密码数据。
- [ ] 创建用户时密码只以哈希形式存储。
- [ ] 管理员可以更新账号状态、角色、显示名称和会员期限。
- [ ] 会员状态计算覆盖未开始、有效、已到期、停用和无记录。
- [ ] 禁用账号和重置密码会撤销全部 Session。
- [ ] 无法禁用或降级最后一个 active 管理员。
- [ ] 管理接口响应不包含 `password_hash`、Session Token Hash 等敏感字段。
- [ ] 管理员后台可以完成列表、创建、编辑、续期、重置密码和撤销会话。
- [ ] 关键操作写入审计日志。
- [ ] Lint、Typecheck、Vitest、Migration 检查、D1 Migration、Next.js Build 和 OpenNext Build 全部通过。

## 分支与 PR

- 分支：`task/004-admin-user-membership-management`
- 独立 Draft Pull Request
- 验收通过后合并，再进入 TASK-005
