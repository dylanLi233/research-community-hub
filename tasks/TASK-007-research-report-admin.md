...
- [x] `research_reports` Migration 可在空数据库应用并可重复执行迁移命令。
- [x] 管理员可以创建、分页筛选、读取和编辑研报。
- [x] 非管理员、未登录或跨源写请求被拒绝。
- [x] Slug 冲突返回 `SLUG_CONFLICT`。
- [x] 非法日期、标签、访问级别和预览模式返回字段级错误。
- [x] 正文清洗错误和警告能够传递给管理员 API。
- [x] 封面和正文素材引用按访问级别执行校验。
- [x] API 不返回 R2 Object Key。
- [x] PATCH 不能直接修改状态、导入来源或审计字段。
- [x] 发布接口写入 `published_at` 并记录审计日志。
- [x] 归档接口通过状态阻止前台展示并记录审计日志。
- [x] 内容哈希在内容无变化时保持稳定，在关键字段变化后改变。
- [x] 自动化测试覆盖字段规范化、媒体提取、素材权限和状态转换。
- [x] Lint、Typecheck、Vitest、Migration 零漂移、两次 D1 Migration、Next.js Build 和 OpenNext Build 全部通过。

> 本任务交付研报管理端领域与 API。Hermes 自动导入和前台会员阅读将在后续任务使用同一张表和安全管道。
