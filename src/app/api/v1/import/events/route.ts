...
import { importEventSchema } from "@/imports/event-validation";
import {
  importMarketEvent,
  ImportEventServiceError,
} from "@/imports/event-service";
import {
  findImportReplay,
...
const ENDPOINT = "/api/v1/import/events";
...
export async function POST(request: NextRequest) {
  ...
  const authorization = await requireApiClientRequest(request, "events:write");
  ...
  const validated = importEventSchema.safeParse(parsedJson);
  ...
  try {
    const result = await importMarketEvent(authorization.db, {
      apiClientId: authorization.client.id,
      idempotencyKey,
      requestHash,
      requestId,
    ...
    const serviceError =
      error instanceof ImportEventServiceError
        ? error
        : new ImportEventServiceError(
            "INTERNAL_ERROR",
            "事件导入暂时不可用",
            500,
          );
